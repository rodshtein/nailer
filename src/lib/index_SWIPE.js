...
export function nailer(node, {
  ...
  function addWheelListener(){
    if ('onwheel' in document) {
      node.addEventListener("wheel", onWheelEvent); // IE9+, FF17+, Ch31+
    } else if ('onmousewheel' in document) {
      node.addEventListener("mousewheel", onWheelEvent); // OLD Type
    } else {
      node.addEventListener("MozMousePixelScroll", onWheelEvent); // Firefox < 17
    }
  }
  ...
  function onWheelEvent(e) {
    ...
    let deltaX = e.deltaX > 0 ? e.deltaX : e.deltaX * -1;
    let deltaY = e.deltaY > 0 ? e.deltaY : e.deltaY * -1;

    wheelDirection = deltaX > deltaY ? 'x' : 'y';
    wheelForce = wheelDirection == 'x' ? e.deltaX : e.deltaY
    // deviceType = wheelForce % 1 ? 'touch' : 'wheel';
    ...
    //   return a;
    // }
    // console.log(graph(wheelForce));
    // console.log(wheelForce);

    node.NAILER.animationId++
    let startPos = node.NAILER.x;

    // invert direction for horizontal scroll
    // it's need too keep correct scroll directions
    let calc = wheelDirection = 'x' ? wheelForce*-1 : wheelForce;

    node.style.transform = `translate(${startPos + calc}px)`
    node.NAILER.x = startPos + calc
  }
  ...
  function onUp() {
    ...
    let _duration = duration > switchPoint &&
        ...
        duration > 1000 &&
         // ↓ if is no scrollable set time to 1000
  ...
  return {
    ...
    destroy() {
      ...
      if ('onwheel' in document) {
        node.removeEventListener("wheel", onWheelEvent); // IE9+, FF17+, Ch31+
      } else if ('onmousewheel' in document) {
        node.removeEventListener("mousewheel", onWheelEvent); // OLD Type
      } else {
        node.removeEventListener("MozMousePixelScroll", onWheelEvent); // Firefox < 17
      }
    }
  };
}